...
public class DataInitializer {
    ...
    private void setupData() {
        ...
        testerService.saveAllTesters(csvDataLoader.loadObjectList(Tester.class, dataConfig.getTesters()));
        ...
        System.out.println("Tester userId: " + 4 + " tested this device: " + "iPhone 4 " + numberOfTimesTesterOnDevice + " times.");

        System.out.println("Tester 1: Mobile Devices: " + testerService.getTester(1L).getMobileDevices().stream().map(Mobile::getDescription).collect(Collectors.toList()));
        System.out.println("Tester 2: Mobile Devices: " + testerService.getTester(2L).getMobileDevices().stream().map(Mobile::getDescription).collect(Collectors.toList()));
        System.out.println("Tester 3: Mobile Devices: " + testerService.getTester(3L).getMobileDevices().stream().map(Mobile::getDescription).collect(Collectors.toList()));
        ...
        System.out.println("Device 1: Testers: " + mobile.getTesters().stream().map(Tester::getFirstName).collect(Collectors.toList()));
        System.out.println("Device 2: Testers: " + mobile2.getTesters().stream().map(Tester::getFirstName).collect(Collectors.toList()));
        System.out.println("Device 3: Testers: " + mobile3.getTesters().stream().map(Tester::getFirstName).collect(Collectors.toList()));


        System.out.println("Bug 1: Get Tester: " + bug1.getFirstName());


        System.out.println("Experienced Testers " + testerService.getMostExperiencedTesters(Country.GB, "3").stream().map(TesterDto::getFirstName).collect(Collectors.toSet()));
